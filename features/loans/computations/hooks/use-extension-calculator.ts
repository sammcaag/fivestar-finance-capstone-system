...
import { GpFactorExtensionRates, GpFactorRenewalExtensionRates } from "../utils/gpfactor-rates";
...
export const useExtensionCalculator = () => {
  ...
  const getGpFactorExtensionRate = (term: number): string => {
    const rate = GpFactorExtensionRates.find((rate) => rate.term === String(term));
  ...
  const getGpFactorRenewalExtensionRate = (reneTerm: number): string => {
    const rate = GpFactorRenewalExtensionRates.find((rate) => rate.reneTerm === String(reneTerm));
  ...
  const getReneTerm = (term: number, oiTerm: string): string => {
    ...
    if (reneTerm > 60) {
      throw new Error("Ren-e Term exceeds 60 months. Please lower the months (term)!");
    } else if (reneTerm < 12) {
      throw new Error("Ren-e Term must be atleast 12 months. Please higher the months (term)!");
    }
  ...
  const getRebates = (
    ...
    if (value < 0) {
      return String(
        Math.abs(value) +
          (Number(extensionGrossProceeds) - Number(extensionTotal) - Number(oiExtension))
      );
    }

    return String(
      Number(extensionGrossProceeds) - Number(extensionTotal) - Number(oiExtension) - value
    );
  };
  ...
  const getNetAmount = (
    ...
    return String(Number(extensionGrossProceeds) - Number(extensionTotal) - Number(oiExtension));
  };
  ...
  const getExtensionMaturityDate = (extensionValueDate: Date, term: number): Date => {
  ...
  const calculateExtensionLoan = (form: FormValues) => {
    ...
    const extensionUnearnedInterest = formatCurrency(
      Number(removeCommas(extensionPrincipalAmount)) - Number(removeCommas(extensionGrossProceeds))
    );
    ...
    const renewalExtensionGpFactor = getGpFactorRenewalExtensionRate(Number(reneTerm));

    //This will return a string | to get the principal amount in renewal extension side
    const renewalExtensionPrincipalAmount = formatCurrency(Number(reneTerm) * monthlyAmortization);
    ...
    const outstandingBalance = formatCurrency(Number(oiTerm) * monthlyAmortization);
    ...
    const newUi = formatCurrency(
      Number(removeCommas(renewalExtensionUnearnedInterest)) - Number(removeCommas(rebates))
    );

    //This will return a string | to get the new gp in references section
    const newGp = formatCurrency(
      Number(removeCommas(renewalExtensionPrincipalAmount)) - Number(removeCommas(newUi))
    );
    ...
    //This will return a date
    const extensionMaturityDate = getExtensionMaturityDate(extensionValueDate, term);
