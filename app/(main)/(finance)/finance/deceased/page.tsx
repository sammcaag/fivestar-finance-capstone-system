...
export default function ClientsVerificationPage() {
  ...
  useEffect(() => {
    document.title = "Deceased Clients | Stella - Five Star Finance Inc.";
  }, []);
  ...
  const pendingClients = useMemo(() => {
    return (
      clientsData?.filter((client) => client.approvalStatus?.toUpperCase() === "DECEASED") ?? []
    );
  }, [clientsData]);
  ...
  return (
    <ContentLayout title="Deceased Clients">
      ...
      <MainTableComp<ClientTableProps>
        title="Deceased Clients Overview"
        description="View and manage the complete list of deceased clients."
        data={pendingClients ?? []}
        isLoading={isLoading}
        columns={clientsColumnDefinition(false)}
        filterColumns={["name", "branchName"]}
        emptyTitle="No Deceased Clients Found"
